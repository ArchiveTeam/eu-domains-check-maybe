...
TRACKER_ID = 'eurid-check'
TRACKER_HOST = 'trackerproxy.meo.ws.iewiewr'
...
LUA_SHA1 = get_hash(os.path.join(CWD, 'eurid-check.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--warc-header', 'eurid-check-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('eurid-check-item: %(item_name)s'),
        ...
        if item_type == 'domain':
            wget_args.extend(['--warc-header', 'eurid-check-domain: ' + item_value])
            wget_args.append('https://whois.eurid.eu/en/search/?domain={}'.format(item_value))
        else:
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='eurid-check'),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
